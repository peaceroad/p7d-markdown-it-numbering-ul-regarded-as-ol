...
- **Design principles**  
  ...
  - Performance – avoid needless `O(n²)` passes, precompute maps when possible.  
  - Maintainability – name functions clearly and comment only when the flow is not obvious.
...
2. **Literal ordered-list normalisation – `normalizeLiteralOrderedLists`**  
   Markdown-it only emits nested `<ol>` when the child numbering starts at `1`. This normaliser scans list-item paragraphs for lines that *look* like ordered markers (based on `detectMarkerType`). When found, it:
   - Splits the paragraph into literal list segments and plain text segments.
   - Creates real `ordered_list` tokens with `_literalList` and `_literalTight` hints so later phases know they were synthesised from paragraph text.
   - Preserves inline/paragraph attributes and keeps indentation-sensitive paragraphs tight unless there was a blank line before them.
   - Merges immediately-following markdown-it generated `<ol>` tokens so there is only one child `<ol>` per list item.
...
3. **Phase 1 – `analyzeListStructure`**  
   ...
   - `markerInfo` (type, sequence count, literal numbers) by calling `detectMarkerType`.
...
4. **Phase 2 – `convertLists` + `simplifyNestedBulletLists`**  
   ...
   - Flattens `ul > li > ol` scaffolding (the “- 1.” pattern). When flattening, it merges marker metadata from parent and child lists, honours `_literalList` so synthetic lists keep their numbering, and fixes tight/loose states (paragraph `hidden` flags) using the `_literalTight` hints emitted by the normaliser.
...
- When flattening `- 1.` patterns, use token `map` data to detect blank lines between the original outer `<li>` blocks. Honour `_literalTight` to prevent fabricated paragraphs from being forcefully shown.
...
- Use `npm run dump:tokens -- <testfile> <index>` or `npm run dump:listinfos -- <testfile> <index>` (see `debug/` scripts) when you need to inspect tokens or Phase 1 `listInfo` output.
- Literal nested list normalisation emits `_literalList` and `_literalTight` flags on tokens. When diagnosing tight/loose issues, print those flags to confirm whether the normaliser created or altered a list.
